...
def emotionPredict(text):
    ...
    if text != "":
        ...
        padded = pad_sequences(sequences, maxLength = max_length, padding = padding_type, truncating = trunc_type)

        testing = np.array(padded)
        predicted_class_label = np.argmax(model.predict(testing), axis = 1)
